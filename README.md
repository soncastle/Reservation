...
- Back
  ...
  - 테스트 및 품질검증
    - 단위 테스트 : Service 로직을 독립적으로 검증하여 로그인, 회원가입 검증, 좌석 예약 비즈니스 규칙 등을 안전하게 보호
    - 통합 테스트 : 회원가입 중복 처리, 좌석 예약 중복 로직, 동시성 문제를 재현·검증하여 데이터 무결성과 안정성을 확보
    - 예외 흐름 검증: ErrorCode 기반 커스텀 예외가 각 시나리오에서 예상한 형태로 동작하는지 테스트하여 오류 응답의 일관성과 신뢰성 보장
  - API
    - 결제기능 : Toss Payment를 활용한 결제 기능 구현
## 적용한 세부 기능 및 디자인
...
-----------------
#### 페이지 및 기능별 수정현황
#13. 결제 기능 추가 / 지도기능 수정 / 전역 예외처리 수정
- Back & Front
  - Toss Payment를 활용한 결제 기능 추가
- Back
  - 회원 가입 관련 동시성 전역 에외처리 수정
  - 결제 단위 테스트 진행
- Front
  - 카카오 지도 API 관련 오류 수정
