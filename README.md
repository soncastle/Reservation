...
- Back
  - 아키텍처 : MVC 패턴을 적용하여 기능별 역할을 명확히 분리하고 데이터 전달의 안정성 확보
  - 보안 : Spring Security를 활용하여 사용자 인증 및 데이터 접근 권한을 안전하게 관리
  - 예외처리 : 전역 예외 처리와 도메인별 예외 클래스를 분리를 통한 오류 대응을 체계화 및 서비스 안정성을 강화
  - RESTFul api :지보수성을 높이기 위해 CRUD 구조와 URI 계층에 맞도록 API를 개선
  - DB
